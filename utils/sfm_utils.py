...
class SfMData:
    def __init__(self, dataset, ref_img="", scale=1, dmin=0, dmax=0, invz=0, render_style="", offset=200):
        self.scale = scale
        self.ref_cam = None
        self.ref_img = None
        ...
        self.offset = 200
        # Detect dataset type
        can_hanle = self.readDeepview(dataset) or self.readLLFF(dataset, ref_img) or self.readColmap(dataset)
        if not can_hanle:
    ...
    def readLLFF(self, dataset, ref_img=""):
        ...
        if not os.path.exists(image_dir) and not os.path.isdir(image_dir):
            return False
        # load R,T
        train_poses, reference_depth, reference_view_id, render_poses, poses, intrinsic, self.webgl = load_llff_data(
            dataset, factor=None, split_train_val=8, render_style=self.render_style
        )
...
def readImagesBinary(path):
    ...
    for i in range(num_reg_images):
        ...
        images[image_id] = {"camera_id": camera_id, "r": r, "t": t, "R": R, "center": center, "path": "dense/images/" + name}
...
def readCamerasBinary(path):
    ...
    for i in range(num_cameras):
        ...
        cams[camera_id] = {"width": width, "height": height, "fx": fx, "fy": fy, "px": px, "py": py}
        # fx, fy, cx, cy
    f.close()
    return cams
...
def buildCamera(W, H, fx, fy, cx, cy):
    return {"width": int(W), "height": int(H), "fx": float(fx), "fy": float(fy), "px": float(cx), "py": float(cy)}
