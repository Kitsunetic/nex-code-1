...
from utils.video_path import deepviewInnerCircle, webGLspiralPath


class OrbiterDataset(Dataset):
    def __init__(self, dataset, ref_img, scale, dmin, dmax, invz, transform=None, render_style="", offset=200, cv2resize=False):
        self.scale = scale
        self.dataset = dataset
        self.transform = transform
        self.sfm = SfMData(
            dataset, ref_img=ref_img, dmin=dmin, dmax=dmax, invz=invz, scale=scale, render_style=render_style, offset=offset
        )
...
def outputCompactMPI(
    mpi_reflection, sfm, planes, model_dir_nolp, layers, sublayers, offset, invz, webpath="runs/html/", web_width=16000
):
    ...
    if not os.path.exists(webpath + outputFile) or True:
        ...
        for key, value in mpi_reflection.items():
            ...
            if len(value.shape) == 4:
                ...
                io.imsave(webpath + outputFile + f"/{key}.png", np.floor(255 * mpi_reflection[key]).astype(np.uint8))
                ...
                for i in range(out.shape[0]):
                    ...
                    io.imsave(webpath + outputFile + f"/basis_{i + 1}.png", np.floor(255 * basis).astype(np.uint8))
...
def render_video(model, dataset, ray, video_path="runs/video/", render_type="default", dataloader=None):
    ...
    if dataset.sfm.dataset_type == "deepview":
        ...
        render_poses = webGLspiralPath(dataset.sfm.ref_img["r"], dataset.sfm.ref_img["t"], dataset.sfm.dmin, dataset.sfm.dmax)
    ...
    for pose_id in render_poses:
        ...
        if not render_type == "nearest":
            ...
            closet = [
                (pt.mean((feature["center"][0] - feature_train["center"][0]) ** 2), feature_train["image"])
                for feature_train in dataloader
            ]
    ...
    os.system(
        'ffmpeg -r 30 -i {}/frame_%04d.png -c:v libx264 -crf 12 -pix_fmt yuv420p -vf pad="width=ceil(iw/2)*2:height=ceil(ih/2)*2" {}/video.mp4 < /dev/null'.format(
...
def measurement(model, sfm, dataloader, ray, write_path=""):
    ...
    return {"name": filenames, "PSNR": psnrs, "SSIM": ssims, "LPIPS": lpipss}
...
def patchtify(img, size):
    ...
    y, x = pt.meshgrid([pt.arange(0, size) + select_row[0], pt.arange(0, size) + select_col[0]])
