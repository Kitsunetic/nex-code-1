...
parser.add_argument(
    "-steps", type=int, default=-1, help="total steps to train. In our paper, we proposed to use epoch instead."
)
parser.add_argument(
    "-tb_saveimage", type=int, default=50, help="write an output image to tensorboard for every <tb_saveimage> epochs"
)
parser.add_argument(
    "-tb_savempi",
    type=int,
    default=200,
    help="generate MPI (WebGL) and measure PSNR/SSIM of validation image for every <tb_savempi> epochs",
)
parser.add_argument(
    "-checkpoint",
    type=int,
    default=100,
    help="save checkpoint for every <checkpoint> epochs. Be aware that! It will replace the previous checkpoint.",
)
parser.add_argument("-tb_toc", type=int, default=500, help="print output to terminal for every tb_toc epochs")

# lr schedule
parser.add_argument(
    "-lrc", type=float, default=10, help="the number of times of lr using for learning rate of explicit basis (k0)."
)
...
parser.add_argument(
    ...
    type=int,
    default=10,
    help="the number of positional encoding in terms of image size. We recommend to set 2^(pos_level) > image_height and image_width",
)
parser.add_argument(
    "-depth_level",
    type=int,
    default=8,
    help="the number of positional encoding in terms number of plane. We recommend to set 2^(depth_level) > layers * subplayers",
)
...
parser.add_argument(
    "-basis_out", type=int, default=8, help="the number of coeffcient output (N in equation 3 under seftion 3.1)"
)
...
parser.add_argument(
    "-ref_img", type=str, default="", help="reference image, camera parameter of reference image is use to create MPI"
)
...
parser.add_argument(
    "-llff_width", type=int, default=1008, help="if input dataset is LLFF it will resize the image to <llff_width>"
)
parser.add_argument(
    "-deepview_width",
    type=int,
    default=800,
    help="if input dataset is deepview dataset, it will resize the image to <deepview_width>",
)
parser.add_argument(
    "-train_ratio",
    type=float,
    default=0.875,
    help="ratio to split number of train/test (in case dataset doesn't specify how to split)",
)
parser.add_argument(
    "-random_split", action="store_true", help="random split the train/test set. (in case dataset doesn't specify how to split)"
)
parser.add_argument("-num_workers", type=int, default=8, help="number of pytorch's dataloader worker")
parser.add_argument(
    "-cv2resize",
    action="store_true",
    help="apply cv2.resize instead of skimage.transform.resize to match the score in our paper (see note in github readme for more detail) ",
)
...
parser.add_argument(
    ...
    type=int,
    default=12,
    help="the number of plane that share the same texture. (please refer to coefficient sharing under section 3.4 in the paper)",
)
...
parser.add_argument(
    ...
    type=int,
    default=16000,
    help="max texture size (pixel) of realtime demo. WebGL on Highend PC is support up to 16384px, while mobile phone support only 4096px",
)
...
# render path
parser.add_argument(
    "-nice_llff", action="store_true", help="generate video that its rendering path matches real-forward facing dataset"
)
...
parser.add_argument(
    "-pretrained", type=str, default="", help="location of checkpoint file, if not provide will use model_dir instead"
)
...
parser.add_argument(
    "-all_gpu",
    action="store_true",
    help="In multiple GPU training, We don't train MLP (data parallel) on the first GPU. This make training slower but we can utilize more VRAM on other GPU.",
)
...
def computeHomographies(sfm, feature, planes):
    ...
    ki = pt.tensor([[fx, 0, px], [0, fy, py], [0, 0, 1]], dtype=pt.float).inverse()

    tt = sfm.ref_cam
    ref_k = pt.tensor([[tt["fx"], 0, tt["px"]], [0, tt["fy"], tt["py"]], [0, 0, 1]])

    planes_mat = pt.Tensor(planes).view(-1, 1, 1)
    return (ref_k @ (Ha + Hb / (-planes_mat - Hc))) @ ki


def computeHomoWarp(sfm, input_shape, input_offset, output_shape, selection, feature, planes, inv=False, inv_offset=False):

    selection = selection.cuda()
    # coords: (sel, 3)
    coords = pt.stack([selection % output_shape[1], selection // output_shape[1], pt.ones_like(selection)], -1).float()
...
def totalVariation(images):
    ...
    tot_var = pt.sum(pt.abs(pixel_dif1), dim=sum_axis) + pt.sum(pt.abs(pixel_dif2), dim=sum_axis)
...
def get_viewing_angle(sfm, feature, ref_coords, planes):
    ...
    ki = (
        pt.tensor([[feature["fx"][0], 0, feature["px"][0]], [0, feature["fy"][0], feature["py"][0]], [0, 0, 1]], dtype=pt.float)
        .inverse()
        .cuda()
    )
...
class Network(nn.Module):
    ...
    def forward(self, sfm, feature, output_shape, selection):
        ...
        warp, ref_coords = computeHomoWarp(sfm, self.shape, sfm.offset, output_shape, selection, feature, self.planes)
...
def getMPI(model, sfm, m=1, dataloader=None):
    ...
    y, x = pt.meshgrid(
        [(pt.arange(0, sh, dtype=pt.float)) / (sh - 1) * 2 - 1, (pt.arange(0, sw, dtype=pt.float)) / (sw - 1) * 2 - 1]
    )
    ...
    y_v, x_v = pt.meshgrid([(pt.linspace(-rangey, rangey, sh_v)), (pt.linspace(-rangex, rangex, sw_v))])
    # viewing [sh_v, sh_w, 2]
    ...
    with pt.no_grad():
        ...
        info = {"mpi_c": mpi_c_sig.cpu(), "mpi_a": mpi_a_sig.cpu(), "mpi_v": mpi_v_tanh.cpu(), "mpi_b": imgs_b.cpu()}
...
def train():
    ...
    sampler_train, sampler_val, dataloader_train, dataloader_val = prepareDataloaders(
        dataset, dpath, random_split=args.random_split, train_ratio=args.train_ratio, num_workers=args.num_workers
    )
    ...
    if args.lrc > 0:
        ...
        optimizer = pt.optim.Adam([{"params": mlp_params, "lr": 0}, {"params": other_params, "lr": 0}])
    else:
    ...
    if args.predict:
        ...
        if not args.no_video:
            ...
            render_video(
                ...
                dataset,
                args.ray,
                os.path.join(runpath, vid_path, args.model_dir),
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % args.tb_saveimage == 0 and args.tb_saveimage > 0:
            with pt.no_grad():
                ...
                writer.add_image(
                    "images/2_mpia",
                    make_grid(F.interpolate(Spec["mpi_a"], (int(mpi_h * 0.3), int(mpi_w * 0.3)), mode="area"), 4),
                    epoch,
                )
                writer.add_image(
                    "images/2_mpic",
                    make_grid(F.interpolate(Spec["mpi_c"], (int(mpi_h * 0.3), int(mpi_w * 0.3)), mode="area"), 4),
                    epoch,
                )
                pt.cuda.empty_cache()
